...
poss(herausnehmen(O,X),S) :- schrank(X), door_open(X,S), contains(X,O,S).
poss(herausnehmen(O,X),S) :- arbeitsplatte(X), contains(X,O,S).
...
contains(X,O,do(A,S)) :-
	A = einstellen(O,X);
	A = abstellen(O,X), arbeitsplatte(X);
...
% Initial situation auto generated by the system.
